...
- **Audit SMTP/alertes pré-prod** : env only (aucun secret hardcodé), destinataires tenant+rôle+prefs, anti-doublon (conflict_alert_sent + dedup_key), audit conflict_stale_alerted, exceptions catchées (dispatch échoué → retente au prochain run ; e-mail échoué → marqué, pas de boucle). NOUVEAU champ honnête `notifications_log.email_delivery` : 'not_configured' | {'sent','failed'} — plus jamais « envoyé » sans preuve ; log « Envoi SMTP non vérifié dans cet environnement ». Script VPS reproductible `backend/scripts/test_conflict_alert_prod.py` (véhicule/chauffeurs/sessions synthétiques, conflict_at -61 min ISOLÉ, seuil prod 60 min INTACT, vérifie in-app/email_delivery/audit/anti-doublon, cleanup). STATUT : PARTIEL — validation production requise (vrai e-mail reçu).
- **Bug produit corrigé** : kill-switch renvoyait 500 quand Navixy refuse raw_command (403 réel avec la clé actuelle) → catch par véhicule, 200 + {errors:3} + audit kill_switch_error.
- **Tests réparés (dérive/bugs de tests, pas de produit)** : mdp superadmin hardcodé obsolète (test_multitenant, test_deploy_regression, test_administration → env) ; test_livre_de_bord aligné données réelles (Jean n'a plus de trajets démo → seed+cleanup) ; modes A/B/C legacy → mixte/masked + C refusé ; matricule test unique (T-9 résiduel supprimé) ; test_phase3 event loop asyncio (import manquant + _loop()/_run()/_ensure_loop autouse — les asyncio.run de modules précédents mettaient la policy à None) ; conftest.py : fixture module autouse purge login_attempts des comptes partagés (le brute force verrouillait en cascade sur la suite complète) ; fix fusion APP+BLE légitime dans test_07/test_E (purge détections+sessions ouvertes avant claim) ; backfill 1 trip test sans vehicle_plate.
- **GATE** : READY FOR PHASE 4 — EXPO (voir rapport chat 18/08). BLE physique + e-mail prod = validations utilisateur, non bloquantes pour démarrer l'app Expo.

### Iteration 38 — Unicité chauffeurs + Alerte conflits 1h + Percée preuve BLE + Prompt Expo (18/08/2026) — SELF-TESTÉ (12/12 cas + 79 pytest PASS)
- **Unicité chauffeurs (tenant)** : e-mail (insensible casse) et matricule (trimé) refusés en doublon → 409 sur POST et PATCH /team/drivers (6 tests PASS). Pas de migration destructive sur doublons existants.
- **Alerte conflits non résolus > 1 h** : nouvel événement `ble.conflict_stale` (catalogue notifications, email par défaut ON, inapp+email_real), fonction `alert_stale_conflicts()` (ble_engine, seuil env CONFLICT_ALERT_AFTER_MIN=60, une seule alerte par conflit via flag conflict_alert_sent + dedup_key, audit conflict_stale_alerted), job scheduler toutes les 10 min (CONFLICT_ALERT_JOB_ID) par tenant. Testé : conflit réel backdaté → 1 alerte, notifications_log + email_planned≥1, pas de double alerte. ⚠️ SMTP_HOST absent du .env PREVIEW → envoi loggé/échoue proprement ; en PROD VPS (SMTP configuré) l'e-mail part réellement. HTML email généralisé (utilise data.link au lieu du texte anomalies codé en dur).
- **PERCÉE PREUVE BLE RÉELLE** : bug réel corrigé — Navixy renvoie code 217 « List contains nonexistent entities » si UN tracker_id de la liste n'existe plus (6/18 traceurs locaux obsolètes : ids 5000-5005 = mocks). Correctif dans driver_beacons.py : `_navixy_tracker_ids()` (tracker/list) + `_read_beacons()` avec repli batch→valides→par-traceur. Résultat réel : **73 enregistrements beacon RÉELS reçus** (véhicule 5-VD 358-707 NEDIR, hardware_id UUID iBeacon 128 bits, rssi, get_time) → **chaîne traceur Teltonika → Navixy → backend PROUVÉE EN RÉEL**. RESTE PARTIEL : aucun beacon n'est encore assigné à un chauffeur (drivers.ble_id vide partout) — l'utilisateur doit assigner le tag physique du chauffeur (fiche chauffeur, format hex libre : MAC ou UUID 32 hex) puis re-lancer scripts/ble_proof.py. Docstring driver_beacons corrigée (ne prétend plus la chaîne prouvée pour le tag chauffeur). Script ble_proof.py : diagnostic traceurs valides/obsolètes + repli.
- **Prompt App Expo** : /app/EXPO_APP_PROMPT.md — cahier des charges complet du projet mobile séparé (18 endpoints réels, flux auth+refresh+must_change_password, 6 écrans, règles métier, BLE natif optionnel dev-build, comptes test). Aucun endpoint inventé.
- Régression : 79 pytest PASS (phase3 + iteration23 + phase_a).
